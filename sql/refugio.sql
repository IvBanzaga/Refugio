...
,`numero` int(11)
,`capacidad` int(11)
,`total_camas` bigint(21)
,`camas_libres` decimal(22,0)
,`camas_pendientes` decimal(22,0)
,`camas_reservadas` decimal(22,0)
);
...
,`fecha_inicio` date
,`fecha_fin` date
,`estado` enum('pendiente','reservada','cancelada')
,`fecha_creacion` timestamp
,`num_socio` varchar(20)
,`usuario_nombre` varchar(50)
,`usuario_apellido1` varchar(50)
,`email` varchar(100)
,`habitacion_numero` int(11)
,`cama_numero` int(11)
,`dias_estancia` int(8)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_habitaciones`  AS SELECT `h`.`id` AS `id`, `h`.`numero` AS `numero`, `h`.`capacidad` AS `capacidad`, count(`c`.`id`) AS `total_camas`, sum(case when `c`.`estado` = 'libre' then 1 else 0 end) AS `camas_libres`, sum(case when `c`.`estado` = 'pendiente' then 1 else 0 end) AS `camas_pendientes`, sum(case when `c`.`estado` = 'reservada' then 1 else 0 end) AS `camas_reservadas` FROM (`habitaciones` `h` left join `camas` `c` on(`h`.`id` = `c`.`id_habitacion`)) GROUP BY `h`.`id`, `h`.`numero`, `h`.`capacidad` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_reservas_completas`  AS SELECT `r`.`id` AS `id`, `r`.`fecha_inicio` AS `fecha_inicio`, `r`.`fecha_fin` AS `fecha_fin`, `r`.`estado` AS `estado`, `r`.`fecha_creacion` AS `fecha_creacion`, `u`.`num_socio` AS `num_socio`, `u`.`nombre` AS `usuario_nombre`, `u`.`apellido1` AS `usuario_apellido1`, `u`.`email` AS `email`, `h`.`numero` AS `habitacion_numero`, `c`.`numero` AS `cama_numero`, to_days(`r`.`fecha_fin`) - to_days(`r`.`fecha_inicio`) AS `dias_estancia` FROM (((`reservas` `r` join `usuarios` `u` on(`r`.`id_usuario` = `u`.`id`)) join `camas` `c` on(`r`.`id_cama` = `c`.`id`)) join `habitaciones` `h` on(`c`.`id_habitacion` = `h`.`id`)) ;
